...
TEST(eof_validation, EOF1_rjump_invalid_destination)
{
    ...
    auto cont =
        eof1_bytecode(rjump(9) + 0 + 0xff + 0 + 0 + OP_CREATE3 + Opcode{0} + OP_POP + OP_STOP, 4,
            {}, 0, embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_rjump_destination);

    // To RETURNCONTRACT immediate
    cont = eof1_bytecode(rjump(5) + 0 + 0 + OP_RETURNCONTRACT + Opcode{0}, 2, {}, 0, embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_rjump_destination);
}
...
TEST(eof_validation, EOF1_rjumpi_invalid_destination)
{
    ...
    // To RETURNCONTRACT immediate
    cont = eof1_bytecode(rjumpi(5, 0) + 0 + 0 + OP_RETURNCONTRACT + Opcode{0}, 2, {}, 0, embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_rjump_destination);
}
...
TEST(eof_validation, EOF1_rjumpv_invalid_destination)
{
    ...
    // To RETURNCONTRACT immediate
    cont =
        eof1_bytecode(rjumpv({5}, 0) + 0 + 0 + OP_RETURNCONTRACT + Opcode{0}, 2, {}, 0, embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_rjump_destination);
}
...
TEST(eof_validation, EOF1_create3_invalid)
{
    // truncated immediate
    const auto embedded = eof1_bytecode(OP_INVALID);
    auto cont = eof1_bytecode(bytecode(0) + 0xff + 0 + 0 + OP_CREATE3, 4, {}, 0, embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::truncated_instruction);

    // last instruction
    cont = eof1_bytecode(bytecode(0) + 0xff + 0 + 0 + OP_CREATE3 + Opcode{0}, 4, {}, 0, embedded);
    ...
    cont = eof1_bytecode(bytecode(0) + 0xff + 0 + 0 + OP_CREATE3 + Opcode{0xff} + OP_POP + OP_STOP,
        4, {}, 0, embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_container_section_index);
}
...
TEST(eof_validation, EOF1_returncontract_valid)
{
    // initcontainer_index = 0
    const auto embedded = eof1_bytecode(OP_INVALID);
    auto cont = eof1_bytecode(bytecode(0) + 0 + OP_RETURNCONTRACT + Opcode{0}, 2, {}, 0, embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::success);

    // initcontainer_index = 1
    const std::array embedded_conts_2 = {embedded, embedded};
    cont =
        eof1_bytecode(bytecode(0) + 0 + OP_RETURNCONTRACT + Opcode{1}, 2, {}, 0, embedded_conts_2);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::success);
...
TEST(eof_validation, EOF1_returncontract_invalid)
{
    // truncated immediate
    const auto embedded = eof1_bytecode(OP_INVALID);
    auto cont = eof1_bytecode(bytecode(0) + 0 + OP_RETURNCONTRACT, 4, {}, 0, embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::truncated_instruction);

    // referring to non-existent container section
    cont = eof1_bytecode(bytecode(0) + 0 + OP_RETURNCONTRACT + Opcode{1}, 4, {}, 0, embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_container_section_index);
    cont = eof1_bytecode(bytecode(0) + 0 + OP_RETURNCONTRACT + Opcode{0xff}, 4, {}, 0, embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_container_section_index);
