...
void call_impl(PARAMS) noexcept
{
    ...
    msg.value =
        (Op == OP_DELEGATECALL) ? state.msg->value : intx::store_be256<evmc::uint256be>(value);
    ...
    if (state.msg->depth >= 1024)
        RETURN_STATUS(EVMC_SUCCESS)  // "Light" failure.

    if (has_value && intx::load_be256(state.host.get_balance(state.msg->recipient)) < value)
        RETURN_STATUS(EVMC_SUCCESS)  // "Light" failure.
...
template void call_impl<OP_CALL>(PARAMS) noexcept;
template void call_impl<OP_STATICCALL>(PARAMS) noexcept;
template void call_impl<OP_DELEGATECALL>(PARAMS) noexcept;
template void call_impl<OP_CALLCODE>(PARAMS) noexcept;


template <Opcode Op>
void create_impl(PARAMS) noexcept
{
    ...
    if (endowment != 0 &&
        intx::load_be256(state.host.get_balance(state.msg->recipient)) < endowment)
        RETURN_STATUS(EVMC_SUCCESS)  // "Light" failure.
    ...
    msg.depth = state.msg->depth + 1;
    msg.create2_salt = intx::store_be256<evmc::bytes32>(salt);
    msg.value = intx::store_be256<evmc::uint256be>(endowment);
    ...
    if (result.status_code == EVMC_SUCCESS)
        stack.top() = intx::load_be256(result.create_address);

    RETURN_STATUS(EVMC_SUCCESS)
}

template void create_impl<OP_CREATE>(PARAMS) noexcept;
template void create_impl<OP_CREATE2>(PARAMS) noexcept;
}  // namespace evmone::instr::core
