...
/// The global, revision-independent, table of error cases of EVM instructions.
constexpr inline std::array<evmc_status_code, 256> error_cases = []() noexcept {
    std::array<evmc_status_code, 256> error{};

    // EVMC_INVALID_MEMORY_ACCESS
    error[OP_RETURNDATACOPY] =
    error[OP_DATALOAD] =
    error[OP_DATACOPY] = EVMC_INVALID_MEMORY_ACCESS;
    // EVMC_BAD_JUMP_DESTINATION
    error[OP_JUMP] = error[OP_JUMPI] = EVMC_BAD_JUMP_DESTINATION;
    // EVMC_STATIC_MODE_VIOLATION
    error[OP_CALL] =
    error[OP_SSTORE] = error[OP_TSTORE] =
    error[OP_CREATE] = error[OP_CREATE2] =
    error[OP_LOG0] = error[OP_LOG1] = error[OP_LOG2] = error[OP_LOG3] =
    error[OP_SELFDESTRUCT] = EVMC_STATIC_MODE_VIOLATION;
    // EVMC_STACK_UNDERFLOW
    error[OP_DUPN] = error[OP_SWAPN] = EVMC_STACK_UNDERFLOW;
    // EVMC_INVALID_INSTRUCTION
    error[OP_INVALID] = EVMC_INVALID_INSTRUCTION;

    return error;
}();

/// Determines if an instruction may still fail after passing the pre-checks
/// of base gas cost and stack heights.
inline constexpr bool has_extra_error_cases(Opcode op) noexcept
{
    return !has_const_gas_cost(op) || (error_cases[op] != EVMC_SUCCESS);
}
